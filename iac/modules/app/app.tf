...
variable "min_tasks" {
  type        = number
  description = "Minimum amount of fargate tasks"
}

variable "max_tasks" {
  type        = number
  description = "Maximum amount of fargate tasks"
}

variable "cpu_size" {
  type        = number
  description = "Should be divisible by 256, where 256 = 0.25 vCPU's. 1024 would equal 1 full vCPU"
}

variable "memory_amount" {
  type        = number
  description = "Amount of fargate task memory (in MiB)"
}
...
module "my_fargate_api" {
  ...
  task_cpu                         = var.cpu_size
  task_memory                      = var.memory_amount
